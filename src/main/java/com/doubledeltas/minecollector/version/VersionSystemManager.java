...
public class VersionSystemManager {
    private final NavigableSet<VersionSystem> registry = new TreeSet<>(Comparator.comparingInt(VersionSystem::getOrder));

    public void register(VersionSystem versionSystem) {
        registry.add(versionSystem);
    }
    ...
    public Version<?> parse(String string) {
        for (Iterator<VersionSystem> it = registry.descendingIterator(); it.hasNext(); ) {
