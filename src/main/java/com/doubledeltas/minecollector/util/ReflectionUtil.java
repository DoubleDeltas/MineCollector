...
public final class ReflectionUtil {
            ...
            throws NoSuchFieldException, NoSuchMethodException, IllegalAccessException
    {
        ...
        for (String fieldName : fieldNames) {
            Class<?> fieldType = curObj.getClass().getField(fieldName).getType();

            String prefix = Boolean.class.isAssignableFrom(fieldType) ? "is" : "get";

            String getterName = prefix + Character.toUpperCase(fieldName.charAt(0))
                    + fieldName.substring(1);
            Method getter = curObj.getClass().getMethod(getterName);
